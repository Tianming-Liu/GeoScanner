...
def create_path_animation_with_labels(graph, final_path, gif_filename='path_progression.gif', duration=0.5):
    images = []
    fig, ax = ox.plot_graph(graph, show=False, close=False, node_size=0, edge_color="k", bgcolor="w")
    start_node = final_path[0][0]
    ...
    # Add the persistent text at the bottom of the figure
    persistent_text = fig.text(0.5, 0.05, f"Street Length: {total_street_length:.2f}km    Route Length: {route_length:.2f}km", ha='center', fontsize=12)

    for i in range(1, len(final_path) + 1):
        plot_single_edge_with_label(graph, final_path[i-1], ax)
        ax.set_title("Path Progression",pad=20, fontsize=16, fontweight='bold')

        # Highlight the start node
        ax.scatter(graph.nodes[start_node]["x"], graph.nodes[start_node]["y"], s=100, c='blue', marker='o', label='Start' if i == 1 else "")
        
        # Remove previous end node marker
        if i > 1:
            ax.collections[-1].remove()
        
        # Highlight the current end node
        end_node = final_path[i-1][1]
        end_node_marker = ax.scatter(graph.nodes[end_node]["x"], graph.nodes[end_node]["y"], s=100, c='green', marker='x', label='Current End')
        
        # Convert the plot to an image array and add to the images list
        fig.canvas.draw()
        image = np.frombuffer(fig.canvas.tostring_rgb(), dtype='uint8')
        image = image.reshape(fig.canvas.get_width_height()[::-1] + (3,))
        images.append(image)
        
        # Remove the current end node marker for the next iteration
        end_node_marker.remove()

    plt.close(fig)  # Close the figure to free memory

    # Generate the GIF
    imageio.mimsave(gif_filename, images, duration=duration)  # duration controls the time between frames

def calculate_total_street_length(graph):
    return sum(data['length'] for u, v, k, data in graph.edges(keys=True, data=True))

def calculate_route_length(graph, path):
    total_length = 0
    missing_edges = []
    for u, v, k in path:
        try:
            total_length += graph[u][v][k]['length']
        except KeyError:
            try:
                total_length += graph[v][u][k]['length']  # 尝试反向边
            except KeyError:
                print(f"Edge ({u}, {v}, {k}) not found in graph, checking reversed edge ({v}, {u}, {k}) not found either")
                missing_edges.append((u, v, k))
    if missing_edges:
        print(f"Missing edges: {missing_edges}")
    return total_length

def plot_single_edge_with_label(graph, edge, ax, route_color="r", route_linewidth=4, route_alpha=0.5):
    u, v, d = edge
    ...
    if "geometry" in data:
        ...
        y.extend((graph.nodes[v]["y"], graph.nodes[v]["x"]))
